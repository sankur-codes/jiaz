...
def sprint(
    wrt: str = typer.Option(
        "status",
        "--wrt",
        "-w",
        help="Display sprint data in specific perspective. Values: issue, owner, status, epic",
    ),
    show: str = typer.Option(
        None,
        "--show",
        "-s",
        help="Comma separated columns to show. If not provided, all columns are shown.",
        show_default=False,
    ),
    output: str = typer.Option(
        ...
        "--output",
        "-o",
        help="Display in a specific format. Values: json, table, csv",
    ),
    ...
    mine: bool = typer.Option(
        False, "--mine", "-m", help="Show only issues assigned to the current user"
    ),
):
