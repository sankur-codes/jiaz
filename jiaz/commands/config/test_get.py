...
def test_get_existing_key(runner: CliRunner, isolated_config_file: Path):
    ...
    result = runner.invoke(
        main_cli_app, ["config", "get", "server_url", "--name", "default"]
    )
...
def test_get_user_token_decoded(runner: CliRunner, isolated_config_file: Path):
    ...
    create_config_file_manually(
        isolated_config_file,
        {"default": {"user_token": encoded}, "meta": {"active_config": "default"}},
    )
    result = runner.invoke(
        main_cli_app, ["config", "get", "user_token", "--name", "default"]
    )
    assert result.exit_code == 0
...
def test_get_key_not_found(runner: CliRunner, isolated_config_file: Path):
    ...
    result = runner.invoke(
        main_cli_app, ["config", "get", "nonexistentkey", "--name", "default"]
    )
...
def test_get_config_not_found(runner: CliRunner, isolated_config_file: Path):
    ...
    result = runner.invoke(
        main_cli_app, ["config", "get", "server_url", "--name", "nonexistent"]
    )
    assert result.exit_code == 0
