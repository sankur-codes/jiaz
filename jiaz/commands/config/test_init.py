...
def test_init_no_existing_config(
    ...
    inputs = iter(
        [
            ...
            "",
            "",
            "",  # Empty string for Gemini API key
        ]
    )
...
def test_init_existing_config_add_new_block(
    runner: CliRunner, isolated_config_file: Path, monkeypatch
):
    create_config_file_manually(
        isolated_config_file,
        {
            "default": {
                ...
                "user_token": encode_secure_value("default_token"),
            },
...
def test_init_config_name_conflict(
    runner: CliRunner, isolated_config_file: Path, monkeypatch
):
    create_config_file_manually(
        isolated_config_file,
...
def test_init_fallback_and_required_prompts(
    runner: CliRunner, isolated_config_file: Path, monkeypatch
):
    create_config_file_manually(
        isolated_config_file,
        {
            "default": {
                "server_url": "http://default.com",
                "user_token": encode_secure_value("default_token_val"),
            },
            "meta": {"active_config": "default"},
        },
    )
    ...
    monkeypatch.setattr(
        "typer.prompt", lambda *args, **kwargs: next(inputs_fallback_server)
    )
    result = runner.invoke(main_cli_app, ["config", "init"])
    assert result.exit_code == 0
    cfg = configparser.ConfigParser()
    ...
    assert (
        decode_secure_value(cfg["fallback_server_test"]["user_token"])
        == "new_specific_token"
    )
    # Fallback for token
    ...
    monkeypatch.setattr(
        "typer.prompt", lambda *args, **kwargs: next(inputs_fallback_token)
    )
    result = runner.invoke(main_cli_app, ["config", "init"])
    assert result.exit_code == 0
    cfg.read(isolated_config_file)
    ...
    assert (
        decode_secure_value(cfg["fallback_token_test"]["user_token"])
        == "default_token_val"
    )
    ...
    assert (
        decode_secure_value(cfg["required_token_block"]["user_token"])
        == "actual_required_token_val"
    )
