...
def init():
    ...
    if not [sec for sec in config.sections() if sec != "meta"]:
        typer.echo(
            "Config file does not exist or contains no blocks. Creating default configuration."
        )
        ...
        if new_config_name in config:
            typer.echo(
                f"Config name '{new_config_name}' already exists. Please choose a different name."
            )
        ...
        if api_key:
            ...
            if not (
                config.has_section("meta")
                and config.has_option("meta", "gemini_api_key")
            ):
                if "meta" not in config:
                    config["meta"] = {}
