...
class TestPromptFormatting:
    ...
    def test_jira_prompt_with_special_characters(self):
        ...
        standardized_desc = "Description with special chars: !@#$%^&*()_+{}[]|\\:;\"'<>,.?/"
...
class TestPromptValidation:
    ...
    def test_prompt_length_and_structure(self):
        ...
        for prompt_name, prompt in [
            ...
            assert 500 < len(prompt) < 3000, f"{prompt_name} prompt length should be reasonable"
            
            # Should have clear section breaks
            assert prompt.count('\n') > 10, f"{prompt_name} prompt should have multiple sections"
            
            # Should have consistent formatting
            assert '"""' in prompt, f"{prompt_name} prompt should use proper section markers"
...
class TestPromptEdgeCases:
    ...
    def test_jira_prompt_with_very_long_content(self):
        ...
        formatted_prompt = JIRA_DESCRIPTION_PROMPT.format(
            standardized_description=long_content,
            terminal_friendly_output=long_content
        )
...
class TestPromptIntegration:
    ...
    def test_prompts_usability_in_ai_utils(self):
        ...
        jira_prompt = JIRA_DESCRIPTION_PROMPT.format(
            ...
            terminal_friendly_output=terminal
        )
