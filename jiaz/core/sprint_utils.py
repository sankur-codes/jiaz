...
def get_epic_data_table(sprint, sprint_issue_keys):
    ...
    for issue_key in sprint_issue_keys:
        try:
            issue_type = get_issue_fields(sprint, sprint.get_issue(issue_key), ["type"])["type"]
            if issue_type == "Epic":
    ...
    for issue_key in sprint_issue_keys:
        try:
            epic_link = get_issue_fields(sprint, sprint.get_issue(issue_key), ["epic_link"])['epic_link']
    ...
    for epic_key in sprint_epics:
        try:
            ...
            assignee = epic_data['assignee'].split(" ")[0] if epic_data['assignee'] != colorize("Unassigned", "neg") else epic_data['assignee']
            reporter = epic_data['reporter'].split(" ")[0] if epic_data['reporter'] != colorize("Unassigned", "neg") else epic_data['reporter']
            
            # Process comments properly
            latest_comment_details = sprint.get_comment_details(epic_data['comments'], epic_data['status'])
...
def analyze_sprint(wrt="status", output="json", config=None, show="<pre-defined>", mine=False):
    ...
    if wrt == "issue":
        ...
        data_table, all_headers = get_epic_data_table(sprint,[strip_ansi(issue[all_headers.index("Issue Key")]) for issue in data_table])
