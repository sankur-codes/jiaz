...
from jiaz.core.config_utils import should_use_gemini, get_gemini_api_key
...
class UnifiedLLMClient:
        ...
        if self.use_gemini:
            ...
            if api_key:
                try:
                    ...
                    typer.echo(colorize(f"❌ Failed to initialize Gemini: {e}. Falling back to Ollama.", "neg"))
                    self.use_gemini = False
                    self._initialize_ollama()
            else:
                typer.echo(colorize("⚠️  No valid Gemini API key found. Using Ollama.", "neu"))
                self.use_gemini = False
                self._initialize_ollama()
        else:
            self._initialize_ollama()
    ...
    def query_model(self, prompt: str, model: Optional[str] = None, **kwargs) -> str:
        ...
        if not self.check_availability():
            ...
            typer.echo(colorize(f"❌ {service_name} is not available. Please check your configuration.", "neg"))
        ...
        try:
            ...
            service_name = "Gemini" if self.use_gemini else "Ollama"
            typer.echo(colorize(f"❌ Error communicating with {service_name}: {e}", "neg"))
            raise typer.Exit(code=1)
