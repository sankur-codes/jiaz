...
class TestConfigFileOperations:
    ...
    @patch("jiaz.core.config_utils.CONFIG_FILE")
    @patch("os.path.exists")
    def test_prepend_warning_to_config_file_not_exists(
        self, mock_exists, mock_config_file
    ):
    ...
    def test_load_config_file_exists(
        self, mock_config_dir, mock_config_file, temp_config_file
    ):
...
class TestGeminiAPIOperations:
    ...
    def test_validate_gemini_api_key_valid(self, mock_gemini):
        ...
        mock_gemini.assert_called_once_with(
            model="gemini-2.5-pro", google_api_key="valid_key", max_retries=1
        )
    ...
    def test_get_gemini_api_key_from_active_config(self):
        ...
        config = configparser.ConfigParser()
        config["meta"] = {"active_config": "default"}
        config["default"] = {"gemini_api_key": encode_secure_value("test_api_key")}
    ...
    def test_should_use_gemini_with_key(self):
        ...
        with patch(
            "jiaz.core.config_utils.get_gemini_api_key", return_value="test_key"
        ):
...
class TestPromptOperations:
    ...
    @patch("typer.prompt")
    @patch("typer.echo")
    def test_prompt_api_key_with_retries_max_attempts(
        self, mock_echo, mock_prompt, mock_validate
    ):
...
class TestConfigCollectionOperations:
    ...
    @patch("typer.echo")
    def test_handle_gemini_api_key_input_success(
        self, mock_echo, mock_encode, mock_prompt
    ):
    ...
    def test_full_config_workflow(self, temp_config_file):
        ...
        with patch("jiaz.core.config_utils.CONFIG_FILE", temp_config_file):
            with patch("jiaz.core.config_utils.CONFIG_DIR", temp_config_file.parent):
                ...
                assert (
                    decode_secure_value(loaded_config["default"]["user_token"])
                    == "test_token"
                )
