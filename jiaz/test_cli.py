...
class TestCLI:
    ...
    def test_config_subcommand_available(self, runner):
        ...
        assert any(word in result.output.lower() for word in ["config", "configuration", "init", "set", "get", "list"])
    ...
    def test_analyze_subcommand_available(self, runner):
        ...
        assert any(word in result.output.lower() for word in ["analyze", "issue", "sprint"])
...
class TestCLIIntegration:
    ...
    def test_full_command_chain_help(self, runner):
        ...
        for command_chain in commands_to_test:
            ...
            assert len(result.output) > 0, f"Command chain {command_chain} produced no output"

    def test_cli_import_structure(self):
        """Test that CLI imports are structured correctly."""
        # Test that we can import the main components
        from jiaz.cli import app, main
        from jiaz.commands import config, analyze
